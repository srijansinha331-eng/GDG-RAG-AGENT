...
try:
    ...
    # Minimal fallback implementations so the demo can run without chromadb.
    class _FakeCollection:
        ...
        def query(self, query_texts, n_results=3):
            # Naive keyword-overlap scorer (higher = more similar)
            ...
            distances = [[1 - s for _, s in top]]  # distance = 1 - similarity
    ...
    # Expose a Client that mirrors chromadb.Client()
    class _ChromadbFallback:
        Client = _FakeClient

    chromadb = _ChromadbFallback
...
class KnowledgeBase:
    ...
    def __init__(self, collection_name: str = "gdg_knowledge"):
        ...
        if _CHROMADB_AVAILABLE and embedding_functions is not None:
            ...
            print("   chromadb or embedding utilities not available — running in fallback mode")
