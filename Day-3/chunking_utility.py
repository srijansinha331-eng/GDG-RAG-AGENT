...
class TextChunker:
    ...
    def __init__(self, chunk_size: int = 500, overlap: int = 50):
        ...
        """
        # Store chunk_size and overlap as instance variables (sanitized)
        self.chunk_size = int(chunk_size)

        # Allow overlap to be provided as an int; fall back to sensible default
        self.overlap = int(overlap) if overlap is not None else int(0.1 * self.chunk_size)

        # Clamp overlap to a reasonable range [0, chunk_size-1]
        if self.overlap < 0:
            self.overlap = 0
    ...
    def chunk_by_words(self, text: str) -> List[Dict]:
        ...
        while start < len(words):
            ...
            # Move start position (with overlap). Ensure integer indices
            next_start = int(end - self.overlap)

            # Prevent infinite loop or no forward progress
            if next_start <= start:
                break
